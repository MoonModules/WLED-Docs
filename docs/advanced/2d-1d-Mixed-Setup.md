...
> As WLED-SR has introduced 2D panels long before upstream WLED, we are still using the original approach for mixed setups. As a benefit, you will be able to create interesting effects by running a 2D effects on "stripes" of a pseudo 2D panel (example: GEQ 2D on a pseudo 16x6 panel -> puts all bars next to each other on the strip)
...
4. Position the "pseudo" 2D panel at the right / top side of your "real" 8x8 matrix panel => offset X = 8, offset Y = 0. Another possibility is to place the "pseudo" panel below your nowmal panel in 2D settings.
