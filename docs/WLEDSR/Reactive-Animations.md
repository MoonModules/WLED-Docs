...
**Note 1:** Effects beginning with ` ♪ ` (`*` in list below) are volume only and run on both ESP32 and ESP8266 (unless they are 2D).    
**Note 2:** Effects beginning with ` ♫ ` (`**` in list below) use FFT (Fast Fourier Transforms) for frequency detection and only run on the ESP32.    
...
See also [Non reactive animations](https://mm.kno.wled.ge/WLEDSR/Non-Reactive-Animations)   
